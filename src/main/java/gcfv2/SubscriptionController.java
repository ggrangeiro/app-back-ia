...
public class SubscriptionController {
    ...
    public HttpResponse<?> listPlans() {
        return HttpResponse.ok(Map.of(
                "plans", List.of(
                        Map.of("id", "FREE", "name", "Gratuito", "price", 0.0, "credits", 0, "generationsLimit", 0,
                                "features", List.of()),
                        Map.of("id", "STARTER", "name", "Starter", "price", 59.90, "credits", 30, "generationsLimit",
                                10, "features", List.of("30 análises de vídeo/mês", "10 gerações de treino/dieta")),
                        Map.of("id", "PRO", "name", "Pro", "price", 99.90, "credits", 80, "generationsLimit", -1,
                                "features",
                                List.of("80 análises de vídeo/mês", "Gerações ilimitadas", "Relatórios PDF")),
                        Map.of("id", "STUDIO", "name", "Studio", "price", 199.90, "credits", 200, "generationsLimit",
                                -1, "features",
                                List.of("200 análises de vídeo/mês", "Gerações ilimitadas", "White Label")))));
    }
    ...
    public HttpResponse<?> getMe(@QueryValue Long userId) {
        return usuarioRepository.findById(userId).map(user -> {
            ...
            return HttpResponse.ok(Map.of(
                    ...
                    "plan", Map.of(
                            "type", user.getPlanType() != null ? user.getPlanType() : "FREE",
                            "status", user.getSubscriptionStatus() != null ? user.getSubscriptionStatus() : "INACTIVE",
                            "renewsAt",
                            user.getSubscriptionEndDate() != null ? user.getSubscriptionEndDate().toString() : null),
                    "usage", Map.of(
                            "credits", user.getCredits() != null ? user.getCredits() : 0,
                            "generations", user.getGenerationsUsedCycle() != null ? user.getGenerationsUsedCycle() : 0,
        ...
        return usuarioRepository.findById(userId).map(user -> {
            ...
            String reason = oldPlan == null || "FREE".equals(oldPlan) ? "SUBSCRIPTION"
                    : (PLANS.get(planId).get("price").toString()
                            .compareTo(PLANS.getOrDefault(oldPlan, PLANS.get("FREE")).get("price").toString()) > 0
                                    ? "UPGRADE"
                                    : "DOWNGRADE");
    ...
    public HttpResponse<?> cancel(@QueryValue Long userId) {
        return usuarioRepository.findById(userId).map(user -> {
            ...
            subscriptionHistoryRepository.save(new SubscriptionHistory(userId, oldPlan, oldPlan, "CANCELLATION"));

            return HttpResponse.ok(Map.of(
                    "message", "Assinatura cancelada. Você mantém acesso até a data de expiração.",
                    "expiresAt",
                    user.getSubscriptionEndDate() != null ? user.getSubscriptionEndDate().toString() : "N/A"));
        }).orElse(HttpResponse.notFound());
    }
