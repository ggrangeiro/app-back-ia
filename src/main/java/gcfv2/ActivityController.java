...
public class ActivityController {
    ...
    public HttpResponse<?> listActivities(
        ...
        if (!"ADMIN".equalsIgnoreCase(requesterRole)) {
            if (!"PERSONAL".equalsIgnoreCase(requesterRole) || !requesterId.equals(managerId)) {
                return HttpResponse.status(HttpStatus.FORBIDDEN)
                        .body(Map.of("message", "Acesso negado. Você não pode visualizar essas atividades."));
            }
        }
        ...
        try {
            List<AtividadeProfessor> activities = activityRepository.findActivitiesFiltered(
                    managerId, professorId, actionType, start, end, pageSize, offset);

            long totalElements = activityRepository.countActivitiesFiltered(
                    managerId, professorId, actionType, start, end);
            ...
            for (AtividadeProfessor activity : activities) {
                ...
                // Metadata é mantido como null na resposta para simplicidade
                // (o JSON string está no banco, mas não parsemos aqui)
    ...
    public HttpResponse<?> getProductivitySummary(
        ...
        try {
            ...
            summary.setTotals(new ProfessorStatsDTO(totalStudents, totalWorkouts, totalDiets, totalAnalysis));
